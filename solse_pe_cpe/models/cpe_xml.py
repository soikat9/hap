...
class CPE:
	...
	def _agregar_informacion_descuentos_globales(self, invoice_id):
		if invoice_id.tiene_retencion:
			...
			etree.SubElement(allowance_charge, (tag.text), currencyID=(invoice_id.currency_id.name), nsmap={'cbc': tag.namespace}).text = str(invoice_id.monto_retencion_base)
	...
	def _agregar_informacion_de_impuestos(self, invoice_id):
		...
		datos = invoice_id.tax_totals_json #.replace("false", "False")
	...
	def _agregar_informacion_montos_totales(self, invoice_id):
		...
		total_redondeo = 0
		for line in invoice_id.invoice_line_ids.filtered(lambda ln: ln.is_rounding_line == True):
	...
	def _agregar_informacion_lineas_comprobante(self, invoice_id):
		...
		for line in invoice_id.invoice_line_ids.filtered(lambda ln: ln.price_subtotal >= 0 and ln.display_type == False and ln.is_rounding_line == False):
