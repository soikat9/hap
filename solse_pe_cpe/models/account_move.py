...
class AccountMove(models.Model):
	...
	tipo_cambio_dolar_sistema = fields.Float("Tipo Cambio ($)", compute="_compute_tipo_cambio_sistema", store=False)
	...
	def fields_view_get(self, view_id=None, view_type='form', toolbar=False, submenu=False):
		...
		if view_type in ['form']:
			parametros = {}
			datos = False
			_logging.info("datosssssssssssssss")
			_logging.info(self._context)
			
			if self._context.get('default_move_type') != 'in_refund':
				return res

			str_productos_ter_cant = res['arch']
			xml_productos_ter_cant = etree.XML(str_productos_ter_cant)
			root = ET.fromstring(res['arch'])

			node_def = xml_productos_ter_cant.xpath("//field[@name='payment_reference']")[0]
			node_def.set("string", "Nota de crédito del proveedor")
				
			res['arch'] = ET.tostring(xml_productos_ter_cant, encoding='utf8', method='xml')
	...
	def _compute_tipo_cambio_sistema(self):
		for reg in self:
			...
			if reg.currency_id.id == moneda_dolar.id:
				tipo_cambio = moneda_dolar._convert(1.000, reg.company_id.currency_id, reg.company_id, reg.invoice_date)
