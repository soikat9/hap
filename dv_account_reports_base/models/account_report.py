...
class AccountReport(models.AbstractModel):
    ...
    def _get_templates(self):
        return {
                'main_template': 'dv_account_reports_base.main_template',
                'main_table_header_template': 'dv_account_reports_base.main_table_header',
                'line_template': 'dv_account_reports_base.line_template',
                'footnotes_template': 'dv_account_reports_base.footnotes_template',
                'search_template': 'dv_account_reports_base.search_template',
                'line_caret_options': 'dv_account_reports_base.line_caret_options',
        }
    ...
    def open_general_ledger(self, options, params=None):
        ...
        action_vals = self.env['ir.actions.actions']._for_xml_id('dv_account_reports_base.action_account_report_general_ledger')
        action_vals['params'] = {
            'options': options,
            'ignore_session': 'read',
        }
    ...
    def get_html_footnotes(self, footnotes):
        template = self._get_templates().get('footnotes_template', 'dv_account_reports_base.footnotes_template')
    ...
    def open_report_export_wizard(self, options):
        ...
        new_wizard = self.with_context(new_context).env['dv_account_reports_base.export.wizard'].create({'report_model': self._name, 'report_id': getattr(self, 'id', 0)})
        view_id = self.env.ref('dv_account_reports_base.view_report_export_wizard').id
        return {
            'type': 'ir.actions.act_window',
            'name': _('Export'),
            'view_mode': 'form',
            'res_model': 'dv_account_reports_base.export.wizard',
            'target': 'new',
    ...
    def get_pdf(self, options):
        ...
        body = self.env['ir.ui.view']._render_template(
            "dv_account_reports_base.print_template",
