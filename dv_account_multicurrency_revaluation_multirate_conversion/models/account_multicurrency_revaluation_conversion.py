    ...
    def _get_sql(self):
        ...
        return """
            SELECT {move_line_fields},
                   ...
                   currency_rate.currency_id                            AS report_currency_id,
            ...
            JOIN res_currency_rate currency_rate ON currency_rate.currency_id = account.multirate_conversion_currency_id AND currency_rate.name = (CASE WHEN move.invoice_date IS NOT NULL THEN move.invoice_date ELSE aml.date END)
            WHERE ((account.currency_id = aml.company_currency_id OR account.currency_id IS NULL) OR (account.internal_type IN ('receivable', 'payable') AND (aml.currency_id = aml.company_currency_id))) AND account.currency_multirate_conversion_affected = TRUE
            {account_query}
            UNION ALL
            -- Add the lines without currency, i.e. payment in company currency for invoice in foreign currency
            SELECT {move_line_fields},
                   CASE WHEN aml.id = part.credit_move_id THEN -part.credit_amount_currency ELSE -part.debit_amount_currency
                   END                                                  AS report_amount_currency,
                   -part.amount * currency_rate.rate                    AS report_balance,
                   CASE WHEN aml.id = part.credit_move_id THEN -part.credit_amount_currency ELSE -part.debit_amount_currency
                   END / custom_currency_table.rate                     AS report_amount_currency_current,
                   CASE WHEN aml.id = part.credit_move_id THEN -part.credit_amount_currency ELSE -part.debit_amount_currency
                   END / custom_currency_table.rate - -part.amount * currency_rate.rate     AS report_adjustment,
                   currency_rate.currency_id                            AS report_currency_id,
                   account.code                                         AS account_code,
                   account.name                                         AS account_name,
                   currency_rate.currency_display_name                          AS currency_code,
